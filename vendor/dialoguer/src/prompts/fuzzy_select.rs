        ...
        loop {
            ...
            match (key, sel, vim_mode) {
                ...
                (Key::Char('\x15'), _, _) => {
                    ...
                    search_term.drain(..byte_indices[cursor]);
                    cursor = 0;
                    term.flush()?;
                }
                (Key::Char('\x17'), _, _) if cursor > 0 => {
                    // Ctrl+W: delete word before cursor
                    let delete_end = byte_indices[cursor];
                    let before = &search_term[..delete_end];
                    let word_end = before.trim_end().len();
                    let word_start = before[..word_end]
                        .rfind(|c: char| c.is_ascii_whitespace())
                        .map(|i| i + 1)
                        .unwrap_or(0);
                    search_term.drain(word_start..delete_end);
                    cursor = search_term[..word_start].chars().count();
                    term.flush()?;
                }
